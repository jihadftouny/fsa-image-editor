...
public class vw_ExtrasContraste extends JDialog {

	public vw_ExtrasContraste(ArrayList<JInternalFrame> frames) {
		setTitle("Contraste Boost");

		setResizable(false);
		setBounds(100, 100, 455, 207);
		getContentPane().setLayout(new MigLayout("", "[200][center][200]", "[33px][24px][24px][44.00px][43.00px]"));

		JLabel lblIntro = new JLabel("Selecione a imagem que fará o Contraste Boost");
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					if (imagem1 != null) {
						Imagem result = Ex1.histogramAtv(imagem1);
