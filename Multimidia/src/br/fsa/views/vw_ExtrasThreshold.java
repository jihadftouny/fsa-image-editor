...
public class vw_ExtrasThreshold extends JDialog {

	public vw_ExtrasThreshold(ArrayList<JInternalFrame> frames) {
		setTitle("Threshold Regional");

		setResizable(false);
		setBounds(100, 100, 455, 207);
		getContentPane().setLayout(new MigLayout("", "[200][center][200]", "[33px][24px][24px][44.00px][43.00px]"));

		JLabel lblIntro = new JLabel("Selecione a imagem que fará o Threshold Regional");
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					if (imagem1 != null) {
						Imagem result = Ex1.thresholdAtv(imagem1);
