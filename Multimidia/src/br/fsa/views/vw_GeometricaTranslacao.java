...
public class vw_GeometricaTranslacao extends JDialog {
	private JTextField txtX;
	private JTextField txtY;

	public vw_GeometricaTranslacao(ArrayList<JInternalFrame> frames) {
		setTitle("Translação");
				...
				if (txtX.getText().matches("[^0-9]") || txtX.getText().isBlank() || txtX.getText().contains(" ") ||
					...
					if (eixoX >= 0 && eixoY >= 0) {
						...
						for (int i = 0; i < frames.size(); i++) {
							...
							if (imagem1 != null) {
								Imagem result = Geometricas.translacao(imagem1, eixoX, eixoY);
